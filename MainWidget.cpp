...
void MainWidget::musicListWidget_refresh()
{
    if(musiclist_index != -1){
        unsigned int index;
        index=static_cast<unsigned int>(musiclist_index);
        ui->musicListNameLabel->setText(u8"歌单 - "+musiclist[index].getName());
        ui->musicListWidget->setMusicList_playing(musiclist[index]);
    }
}
...
void MainWidget::namelist_delete()
{
    ...
    unsigned int pos1;
    pos1=static_cast<unsigned int>(pos);
    musiclist[pos1].remove_SQL_all();//先从数据库中删除歌曲信息
    //移除歌单
    int i=0;
...
void MainWidget::updatePosition(qint64 position)
{
    ...
    ui->positionLabel1->setText(formatTime(position));
    ui->positionLabel2->setText(formatTime(player->duration()));
...
void MainWidget::updateDuration(qint64 duration)
{
    ...
    if(!(static_cast<int>(duration) > 0)) {
        //无音乐播放时，界面元素
        ui->infoLabel1->setText("歌曲名");
        ui->infoLabel2->setText("艺术家");
        ui->infoLabel3->setText("音乐时长");
        mySystemTray->setToolTip(u8"MusicPlayer · By szy_zsy");
...
void MainWidget::updateInfo()
{
    if (player->isMetaDataAvailable()) {
        ...
        ui->lyricWidget->process(ui->playListWidget->musicList.music[index1].getLyricFile());
    }
}
...
void MainWidget::init_systemTrayIcon()
{
    ...
    mySystemTray->setToolTip(u8"MusicPlayer · By sxy_zsy");
    ...
    if(dialog->exec()==QDialog::Accepted){
        for(int i=0;i<num;i++){
            if(results[i]){
                unsigned int index;
                index=static_cast<unsigned int>(musiclist_index);
                musiclist[index].addMusic(ui->localMusicWidget->musicList.getMusic(i));
            }
        }
        musicListWidget_refresh();
    }
...
void MainWidget::on_musicListWidget_doubleClicked(const QModelIndex &index)
{
    playlist->clear();
    unsigned int index1;
    index1=static_cast<unsigned int>(musiclist_index);
    musiclist[index1].addToPlayList(playlist);
    ui->playListWidget->setMusicList_playing(musiclist[index1]);
    int i=index.row();
    playlist->setCurrentIndex(i);
    player->play();
    ui->stackedWidget->setCurrentIndex(0);//跳转到当前播放列表
...
void MainWidget::on_btnSortSinger_4_clicked()
{
    unsigned int index;
    index=static_cast<unsigned int>(musiclist_index);
    musiclist[index].sort_by(AUTHOR);
    musicListWidget_refresh();
}
...
void MainWidget::on_btnAbout_clicked()
{
    QMessageBox::about(this,u8"关于","MusicPlayer | 一款轻量级的本地音乐播放器\n"
                                   "作者：沈星宇，张思源\n\n"
                                   "【歌词文件说明】\n"
                                   "需与对应歌曲MP3在同目录且同名（.lry文件）\n"
                                   "【快捷键说明】\n"
                                   "播放/暂停： 空格\n"
                                   "上一曲/下一曲：Alt键+方向键←/→\n"
                                   "【添加本地音乐】\n"
                                   "法一：直接拖拽至软件界面内"
                                   "法二：点击本地音乐界面的添加按钮（Ctrl键+O）\n"
                                   "【音乐文件类型】\n"
                                   "添加过程中会自动过滤得到可播放的文件类型（.mp3/.flac/.mpga文件），所以添加时无需考虑文件类型，使用\"Ctrl+A\"选择文件夹内全部文件添加即可\n"
                                   "【特别鸣谢】：南京大学TJL同学的博客（文档中有链接）\n"
                                   );
}
